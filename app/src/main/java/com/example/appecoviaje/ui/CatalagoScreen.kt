...
import androidx.lifecycle.viewmodel.compose.viewModel
import com.ecoviaje.app.model.Viaje
import com.ecoviaje.app.viewmodel.CatalogoViewModel
...
// Componente individual para mostrar un viaje
@Composable
fun ViajeCard(viaje: Viaje) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(12.dp)) {
            // Uso del método polimórfico 'descripcion()'
            Text(text = viaje.descripcion(), style = MaterialTheme.typography.titleMedium)
            Text(text = "Precio por persona: $${viaje.precioPorPersona}", style = MaterialTheme.typography.bodyMedium)
        }
    }
}